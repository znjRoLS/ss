            ...
            },
            {
                ...
                switch (currentTokenType)
                {
                    case PUB_EXT:
                        ...
                        return LINE_END;
                    case DIRECTIVE:
                        if (currentSectionType != DATA && currentSectionType != BSS)
                        {
                            throw runtime_error("Not allowed to write directives not in DATA or BSS section!");
                        }
...
{
    if (directiveName != ".align" && directiveName != ".skip" && currentSectionType == BSS)
    {
        throw runtime_error("Not allowed to write directives that initialize memory in bss");
    }
...
void Compiler::HandleInstruction(string instructionName, string currentSection, queue<string> &tokens, u_int32_t &locationCounter)
{
    ...
    if (regex_match(instructionName, base_match, tokenParsers[INSTRUCTION])) {
        ...
        bool setFlags = base_match[3].matched;
